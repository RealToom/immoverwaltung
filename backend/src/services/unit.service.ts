...
export async function createUnit(
  companyId: number,
  propertyId: number,
  data: { number: string; floor: number; area: number; rent: number; type?: UnitType; status?: UnitStatus; tenantId?: number | null }
) {
  await verifyPropertyOwnership(companyId, propertyId);
...
export async function updateUnit(
  companyId: number,
  id: number,
  data: Partial<{ number: string; floor: number; area: number; rent: number; type: UnitType; status: UnitStatus; tenantId: number | null }>
) {
  const unit = await prisma.unit.findUnique({
    where: { id },
