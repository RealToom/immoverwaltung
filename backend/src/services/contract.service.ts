...
export async function getContract(companyId: number, id: number) {
  const contract = await prisma.contract.findFirst({
    ...
    include: {
      ...
      property: { select: { id: true, name: true, street: true, zip: true, city: true } },
...
export async function createContract(companyId: number, data: CreateContractData) {
  ...
  if (data.status === "AUSLAUFEND" || data.status === "GEKUENDIGT") {
    notifyContractStatusChange(companyId, {
...
export async function updateContract(companyId: number, id: number, data: UpdateContractData) {
  ...
  if (data.status && data.status !== existing.status && (data.status === "AUSLAUFEND" || data.status === "GEKUENDIGT")) {
    notifyContractStatusChange(companyId, {
      tenantName: contract.tenant.name,
      propertyName: contract.property.name,
      status: data.status,
