...
async function main() {
  ...
  const properties = await Promise.all([
    prisma.property.create({ data: { name: "Residenz Am Park", address: "Parkstraße 12, 10115 Berlin", status: "AKTIV", companyId: company.id } }),
    prisma.property.create({ data: { name: "Sonnenhof Apartments", address: "Sonnenallee 45, 12045 Berlin", status: "AKTIV", companyId: company.id } }),
    prisma.property.create({ data: { name: "Lindenhaus", address: "Lindenstr. 8, 80331 München", status: "AKTIV", companyId: company.id } }),
    prisma.property.create({ data: { name: "Gewerbepark Ost", address: "Industriestr. 22, 60329 Frankfurt", status: "WARTUNG", companyId: company.id } }),
    prisma.property.create({ data: { name: "Villa Rheinblick", address: "Rheinufer 3, 50667 Köln", status: "AKTIV", companyId: company.id } }),
  ]);
  ...
  const allUnits = [...units1, ...units2, ...units3, ...units4, ...units5];
  console.log(`Units: ${allUnits.length} created`);
